...
const SlidingAuthForm = () => {
  ...
  const handleRegister = async (values) => {
    try {
      const response = await axios.post('http://localhost:3000/authentication/register', values);
  ...
  const handleLogin = async (values) => {
    try {
        ...
        if (!response.data || !response.data.accessToken || !response.data.userData) {
          ...
          <div className={`container ${isLogin ? '' : 'right-panel-active'}`}>
            <div className="form-container sign-up-container">
              <div>
                ...
                <Form form={registerForm} name="register-form" onFinish={handleRegister} layout="vertical">
            ...
            <div className="form-container sign-in-container">
              <div>
                ...
                <Form form={loginForm} name="login-form" onFinish={handleLogin} layout="vertical">
                  <Form.Item name="email" rules={[{ required: true, message: 'Nhập email!' }, { type: 'email', message: 'Email không hợp lệ!' }]}>
                    <Input prefix={<UserOutlined />} placeholder="Email" />
                  </Form.Item>
                  <Form.Item name="password" rules={[{ required: true, message: 'Hãy nhập mật khẩu!' }]}>
